...
export default function Home() {
  return <main>
    ...
    <section id="members" className="section members"><p className="section-label">02 / Members</p><div><div className="member-grid">{members.map(([name, role, text], i) => <article className="member" key={name}><div className="portrait"><span>{String(i + 1).padStart(2, "0")}</span></div><h3>{name}</h3><p className="role">{role}</p><p>{text}</p></article>)}</div><article className="professor-profile"><div><p className="section-label">Professor / 최유주</p><h3>최유주 교수</h3><p>서울미디어대학원대학교 인공지능응용소프트웨어학과<br /><a href="mailto:yjchoi@smit.ac.kr">yjchoi@smit.ac.kr</a> · 02-6953-3154</p></div><div><p><b>학력</b><br />이화여자대학교 전자계산학과 이학사 · 이학석사<br />이화여자대학교 과학기술대학원 컴퓨터학과 공학박사</p><p><b>연구 전문분야</b><br />Computer Graphics, Image Analysis, Human-Computer Interaction, Smart Surveillance, Augmented Reality, Dynamic Projection Mapping, AI-based Gesture Analysis, AI-based 3D Reconstruction</p><p><b>주요 경력</b><br />서울미디어대학원대학교 교수 (2010–현재)<br />서울벤처정보대학원대학교 조교수 (2005–2010)<br />포스데이터 주식회사 기술연구소 주임연구원</p></div></article></div></section>
