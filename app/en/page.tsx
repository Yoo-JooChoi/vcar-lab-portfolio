...
export default function EnglishHome() {
  return <main>
    ...
    <section id="about" className="section about"><p className="section-label">01 / About</p><div className="section-content"><h2>Expanding reality<br />through visual technology.</h2><div className="body-copy"><p>The Visual Computing &amp; Augmented Reality Lab. investigates 3D stereoscopic visualization, motion analysis, HCI, AR, and information-visualization techniques grounded in computer graphics and image processing.</p><p>We also study core technologies for augmented reality, which seamlessly aligns virtual objects and information with the physical world, and develop application systems that bring those technologies into use.</p></div></div></section>
    ...
    <section id="projects" className="section projects"><p className="section-label">03 / Projects</p><div className="project-list">{projectsEn.map(([number, title, text, year]) => <article className="project" key={number}><span>{number}</span><div><h3>{title}</h3><p>{text}</p></div><time>{year}</time></article>)}</div></section>
